...
void clean( void )
{
    ...
    int l_ret = munmap( shmm, sizeof( *shmm ) );
    if ( l_ret )
...
void send_board(int scl) {
    dprintf(scl, "------------------\n");
    dprintf(scl, "------------------\n");
    for(int i = 0; i < 5; i++) {
        for(int j = 0; j < 5; j++) {
            dprintf(scl, "%c", shmm->field[i][j]);
        }
        dprintf(scl, "\n");
     }
      dprintf(scl, "------------------\n");
...
static void client_handle(int scl) {
    ...
    while(shmm->count < 2) {
        dprintf(scl, "Prosim o strpeni kliet c. %d\n", shmm->count);
        usleep(3000*1000);
    }
    dprintf(scl, "Hra zacina");
    ...
    dprintf(scl, "Zadaj tah\n");
     ...
     shmm->field[atoi(row)][col_i] = my;

    send_board(scl);

     sem_post(&shmm->players[1-index]);
        ...
        while ( 1 ) // wait for new client
        {
            ...
            { // new client?
                ...
                close(l_sock_client);
                    ...
                    dprintf(l_sock_client, "Nie je miesto.");
                    close(l_sock_client);
                }
