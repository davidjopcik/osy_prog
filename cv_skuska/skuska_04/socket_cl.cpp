...
int main( int t_narg, char **t_args )
{
    ...
    char file[4096];
    char f_name[128];
    sprintf(buf, "DAY %s", day);
    sprintf(f_name, "%s.png", day);

    log_msg(LOG_INFO, "Tu som\n");


    write(l_sock_server, buf, strlen(buf));
    int f = open(f_name, O_CREAT | O_WRONLY | O_TRUNC, 0644);

    while(1) {
        int n = read(l_sock_server, file, sizeof(file));
    ...
    log_msg(LOG_INFO, "Sirka je: %s\n", pic_size);
    log_msg(LOG_INFO, "Nazov: %s\n", f_name);

    pid_t p = fork();
    if(p == 0) {
        
        execlp("display", "display", "-resize", pic_size, f_name, nullptr);
        //log_msg(LOG_INFO, "Otvaram obrazok: %s\n", f_name);
        close(l_sock_server);
        _exit(0);
    }
    else{
