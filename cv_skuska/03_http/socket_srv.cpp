...
void parse_string() {
    ...
    char *get = strtok(line, "/");
    char *cmd = strtok(NULL, " ");
    sprintf(first_arg, "%s", strtok(cmd, "*"));
    sprintf(second_arg, "%s", strtok(NULL, "\0"));
}

void run_exec(int scl) {
    pid_t p = fork();
    if (p == 0)
    {
        dup2(scl, STDOUT_FILENO);
        execlp(first_arg, first_arg, second_arg, NULL);
        _exit(0);
    }
    else if (p > 0)
    {
...
void client_handle(int scl) {
    ...
    parse_string();
    printf("CMD: %s %s", first_arg, second_arg);
    fflush(stdout);
...
int main( int t_narg, char **t_args )
{
    ...
    sem = sem_open("/mysem", O_CREAT | O_TRUNC, 0664);
