...
void client_handle2(int scl) {
    int n = read(scl, buf, sizeof(buf)-1);
    if (n <= 0) return;
    buf[n] = '\0';
...
void client_handle(int scl) {
    ...
    printf("Riadok: %s\n", firstLine);
    char *firstWord = strtok(firstLine, "/");
    printf("Prvy string: %s\n", firstWord);
    char *secondString = strtok(NULL, "\0");
    secondString = strtok(secondString, " ");
    printf("command: %s\n", secondString);
    sprintf(firstcmd, "%s", strtok(secondString, "*"));
    sprintf(secondcmd, "%s", strtok(NULL, "\0"));
    printf("CMD:%s %s\n", firstcmd, secondcmd);
    ...
    if (p == 0)
    {
        dup2(scl, STDOUT_FILENO);
            ...
            execlp(firstcmd, firstcmd, secondcmd, NULL);
        /*   dprintf(scl,
  "HTTP/1.1 200 OK\r\n"
