...
void init_queue(void)
{
    ...
    // binárny semafor na pauzu/resume – 1 = beží, 0 = pauza
...
// return 0 = OK, -1 = timeout, -2 = chyba
int consumer(char *item)
{
    if (g_mode == MODE_SHM)
    {
        // pauza – všetci consumeri čakajú na pause_sem
        sem_wait(pause_sem);
        sem_post(pause_sem);

        // timeout 5 sekúnd na semafor full
        struct timespec ts;
    ...
    {
        // MQ – timeout cez mq_timedreceive
        struct timespec ts;
        if (clock_gettime(CLOCK_REALTIME, &ts) == -1)
        {
            log_msg(LOG_ERROR, "clock_gettime failed");
...
void producer(char *item)
{
    if (g_mode == MODE_SHM)
    {
        ...
        // globálne číslovanie – total_produced
...
void run_consumer_client(int sock)
{
    ...
    while (1)
    {
        ...
        // SHM: v buffri je už očíslovaný text "N. ..."
        // MQ: je tam to, čo poslal producent
        char sendbuf[256];
