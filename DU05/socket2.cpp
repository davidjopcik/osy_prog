...
typedef enum {PRODUCER, CONSUMER} Role;
...
int main( int t_narg, char **t_args )
{
    ...
    for ( int i = 1; i < t_narg; i++ )
    {
        ...
        if ( !strcmp(t_args[i], "producer") ) {
            role = PRODUCER;
        }
        if ( !strcmp(t_args[i], "consumer") ) {
            role = CONSUMER;
        }
    }
    ...
    // CHOOSE ROLE
    const char *role_str = (role == PRODUCER) ? "producer\n" : "consumer\n";
    if (write(l_sock_server, role_str, strlen(role_str)) <= 0) {
    log_msg(LOG_ERROR, "Cannot send role to server.");
    exit(1);
    }
