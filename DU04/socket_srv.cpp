...
//***************************************************************************
// jednoduchý shared buffer chránený 3 semaformi
...
static sem_t g_sem_mutex;

static void enqueue_line( const char *s )
{
    sem_wait( &g_sem_empty );
    sem_wait( &g_sem_mutex );

    strncpy( g_queue[g_q_tail], s, MAX_LINE - 1 );
...
static void dequeue_line( char *out )
{
    sem_wait( &g_sem_full );
    sem_wait( &g_sem_mutex );

    strncpy( out, g_queue[g_q_head], MAX_LINE );
    out[MAX_LINE-1] = '\0';
...
//***************************************************************************
// zachovaný názov: child_serve (teraz je to vláknová obsluha klienta)

void *child_serve( void *parg )
{
    thread_arg_t *a = (thread_arg_t*)parg;
    int l_sock_listen = a->l_sock_listen;
...
{
    // výrobca: prijíma riadky
    for (;;)
    {
        if (!recv_line(l_sock_client, line, sizeof(line))) break;
        if (strlen(line) == 0) {            // prázdny riadok ignoruj
            send_str(l_sock_client, "OK\n");
            continue;
        }
        enqueue_line(line);                  // vlož do zdieľanej fronty
        send_str(l_sock_client, "OK\n");     // potvrď klientovi
    }
}
    else if ( !strcmp( line, "consumer" ) )
    {
        for (;;)
        {
            ...
            if ( !recv_line( l_sock_client, line, sizeof(line) ) ) break; // očakáva "OK"
        }
    }
    else
    {
...
int main( int t_narg, char **t_args )
{
    ...
    while ( 1 )
    {
        ...
        {
            cl_num++;
            sockaddr_in l_rsa; int l_rsa_size = sizeof( l_rsa );
            ...
            arg->l_sock_listen = l_sock_listen;
